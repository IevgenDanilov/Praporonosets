...
function addToFavorites(id) {
  ...
  var domElements = JSON.parse(window.localStorage.getItem('favorites')) || [];
  for (var _key in favorites) {
    if (Object.hasOwnProperty.call(favorites, _key)) {
      var element = favorites[_key];
  ...
  var uniqArr = domElements.filter(function (el, ind) {
    return ind === domElements.indexOf(el);
  });
